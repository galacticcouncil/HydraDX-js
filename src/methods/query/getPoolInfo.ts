...
export async function getPoolInfo() {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      allPools.forEach(async ([key, value]) => {
      ...
      for (let i = 0; i < allPools.length; i++) {
        ...
        const poolAssetsAmount = await getPoolAssetsAmounts(poolAssets[0].toString(), poolAssets[1].toString());
        const assetPrices = getAssetPrices(tokenTradeMap, parsedPoolsList, poolAssetAmounts);

        if (poolAssetsAmount && poolAssetsAmount.asset1 && poolAssetsAmount.asset2) {
          const asset1Price = (new BigNumber(poolAssetsAmount.asset1)).multipliedBy(assetPrices[poolAssets[0]]).dividedBy(priceUnit);
          const asset2Price = (new BigNumber(poolAssetsAmount.asset2)).multipliedBy(assetPrices[poolAssets[1]]).dividedBy(priceUnit);
