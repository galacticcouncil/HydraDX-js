...
test('Test getMarketcap at specific block', async () => {
    const api = await Api.initialize({}, process.env.WS_URL);
    const alice = getAliceAccount();
    ...
    const result = await api.hydraDx.query.getPoolAssetsAmounts(asset1.toString(), asset2.toString(), BLOCK_HASH);
    ...
    let poolInfo = await api.hydraDx.query.getPoolInfo(BLOCK_HASH);
    const pools = Object.keys(poolInfo.poolInfo);
    let newPool:any = [];
    await Promise.all(pools.map(async (pool) => {
        const ObjPool = poolInfo.poolInfo[pool];
        const poolResult = await api.hydraDx.query.getPoolAssetsAmounts(ObjPool.poolAssets[0].toString(), ObjPool.poolAssets[1].toString(), BLOCK_HASH);
        const poolTotal = parseInt(poolResult.asset1) + parseInt(poolResult.asset2);
